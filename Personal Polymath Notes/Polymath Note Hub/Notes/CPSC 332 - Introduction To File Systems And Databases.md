---
NoteType: Theory
NoteCreation: 2026-01-21
NoteTags:
---
**REFLECTION** *What Did You Learn, Understand, Research?*

**CONNECTION** *What Is Directly Related To This?*
[[Landing Page]]
**SYNTHESIS** *What Is Indirectly Related To This? At Least Three Things*

****
**Syllabus**
* Data designing and implementation will be part of the class
* Textbook is required for this class
* Utilizing MySQL
* Late assignments will not be graded, turn in on-time.
* Utilizing lockdown browser
* Possible curve when the circumstance allows
* Linked-in learning is going to be used for homework
* Limit use of generational AI; don't rely on it.
* Only choose to be absent five days.
* 
